...
class MockLLM:
    ...
    def chat(self, messages: list) -> str:
        ...
        # 先检查SubAgent角色（更具体的匹配，优先于Leader）
        if "数据研究员" in sys_content:
...
只输出 LEVEL1 / LEVEL2 / LEVEL3，不要其他内容："""
    ...
    def check(self, user_input: str) -> dict:
        ...
        # 简单规则快速判断（减少API调用）
        greeting_patterns = [r"^你好", r"^在吗", r"^hi$", r"^hello$", r"^谢谢", r"^再见"]
        for p in greeting_patterns:
            if re.search(p, user_input, re.IGNORECASE):
                return {"pass": False, "level": "chat", "direct_response": "你好！我是XXN Agent，可以帮你写代码、做分析、解问题。直接告诉我你的需求即可。"}
